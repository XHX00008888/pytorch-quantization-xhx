...
def search(M):
    ...
    while True:
        ...
        print("n=%d, Mo=%f, approx=%d, result=%d, error=%f" % (n, Mo, approx_result, result, error))
...
class QConv2d(QModule):
    ...
    def freeze(self, qi=None, qo=None):
        ...
        self.conv_module.weight.data = self.qw.quantize_tensor(self.conv_module.weight.data) - self.qw.zero_point
...
class QLinear(QModule):
    ...
    def freeze(self, qi=None, qo=None):
        ...
        self.fc_module.weight.data = self.qw.quantize_tensor(self.fc_module.weight.data) - self.qw.zero_point
...
class QBN(QModule):
    ...
    def __init__(self, bn_module, qi=False, num_bits=None, ):
        super(QBN, self).__init__(qi=qi, num_bits=num_bits)
        self.qs = QParam(num_bits=num_bits)
        self.qm = QParam(num_bits=num_bits)
        self.bn_module = bn_module
    ...
    def freeze(self, qi=None):
        ...
        if qi is not None:
            self.qi = qi
        self.bn_module.running_mean = self.qm.quantize_tensor(self.bn_module.running_mean)
        self.bn_module.running_var = self.qs.quantize_tensor(self.bn_module.running_var)
    ...
    def forward(self, x):
        ...
        if self.training:
            y = x.permute(1, 0, 2, 3)  # NCHW -> CNHW
            y = y.contiguous().view(self.bn_module.out_channels, -1)  # CNHW -> C,NHW
            # mean = y.mean(1)
            # var = y.var(1)
            mean = y.mean(1).detach()
            var = y.var(1).detach()
